...
/*--- MPI initialization, and buffer setting ---*/
    ...
    {
        historyFile.open("NativeHistoryAdjoint.dat", std::ios::out);
        historyFile2.open("NativeHistoryAdjointFSI.dat", std::ios::out);
    }
    else
    {
    ...
    {
        std::cout << iMarker << " " << geometry->GetMarkersMoving(iMarker) << " " << geometry->GetnMarkers() << std::endl;
...
void NativeSolidSolver::writeSolution(double currentTime, double lastTime, double currentFSIIter, unsigned long ExtIter, unsigned long NbExtIter)
{
    ...
    if (config->GetUnsteady() == "YES" || config->GetKindProblem() == "ADJOINT")
    {
        if (ExtIter % DeltaIter == 0 || ExtIter == NbExtIter)
        {
            ...
            if (structure->GetnDof() == 1)
            {
                restartFile << "\"Time\"" << "\t" << "\"Displacement\"" << "\t" << "\"Velocity\"" << "\t" << "\"Acceleration\"" << "\t" << "\"Acceleration variable\"" << std::endl;
            ...
            {
                restartFile << "\"Time\"" << "\t" << "\"Displacement 1\"" << "\t" << "\"Displacement 2\"" << "\t" << "\"Velocity 1\"" << "\t" << "\"Velocity 2\"" << "\t" << "\"Acceleration 1\"" << "\t" << "\"Acceleration 2\"" << "\t" << "\"Acceleration variable 1\"" << "\t" << "\"Acceleration variable 2\"" << std::endl;
            ...
            else if (config->GetUnsteady() == "HARMONIC")
            {
                ...
                for (unsigned short iInst = 0; iInst < nInst; iInst++)
                {
                    historyFile2 << std::fixed
                                 ...
                                 << std::setw(10) << FSIter
                                 << std::setw(15) << (integrator->GetSolver()->GetDisp())[iInst]
                                 << std::setw(15) << (integrator->GetSolver()->GetVel())[iInst]
                                 << std::setw(15) << (integrator->GetSolver()->GetAcc())[iInst] << std::endl;
            ...
            else if (config->GetUnsteady() == "HARMONIC")
            {
                ...
                for (unsigned short iInst = 0; iInst < nInst; iInst++)
                {
                    ...
                    historyFile2 << std::fixed
                                 ...
                                 << std::setw(10) << FSIter
                                 << std::setw(15) << (integrator->GetSolver()->GetDisp())[iInst]
                                 << std::setw(15) << (integrator->GetSolver()->GetDisp())[iInst + nInst]
                                 << std::setw(15) << (integrator->GetSolver()->GetVel())[iInst]
                                 << std::setw(15) << (integrator->GetSolver()->GetVel())[iInst + nInst]
                                 << std::setw(15) << (integrator->GetSolver()->GetAcc())[iInst]
                                 << std::setw(15) << (integrator->GetSolver()->GetAcc())[iInst + nInst]
                                 << std::setw(15) << Energy
                                 << std::setw(15) << (integrator->GetSolver()->GetOmega())
                                 << std::setw(15) << (integrator->GetSolver()->GetDeltaOmega()) << std::endl;
                    currentTime += config->GetStopTime() / nInst;
                }
            }
            else
            ...
            else if (config->GetUnsteady() == "HARMONIC")
            {
                ...
                for (unsigned short iInst = 0; iInst < nInst; iInst++)
                {
                    ...
                    historyFile2 << std::fixed
                                 ...
                                 << std::setw(10) << FSIter
                                 << std::setw(15) << Energy
                                 << std::setw(15) << integrator->GetSolver()->GetStiffnessDerivative(0) + dJdkh
                                 << std::setw(15) << integrator->GetSolver()->GetStiffnessDerivative(1) + dJdka
                                 << std::setw(15) << dampingDer
                                 << std::setw(15) << integrator->GetSolver()->GetDampingDerivative(1)
                                 << std::setw(15) << integrator->GetSolver()->GetMassDerivative(0) + integrator->GetSolver()->GetImbalanceDerivative() * structure->Get_S() / structure->Get_m()
                                 << std::setw(15) << integrator->GetSolver()->GetMassDerivative(1)
                                 << std::setw(15) << integrator->GetSolver()->GetImbalanceDerivative() / structure->Get_m() << std::endl;
                    historyFile << std::scientific
                                << std::setw(10) << currentTime
                                << std::setw(10) << FSIter
                                << std::setw(20) << Energy
                                << std::setw(20) << integrator->GetSolver()->GetAdjLoads()[iInst]
                                << std::setw(20) << integrator->GetSolver()->GetAdjLoads()[iInst + nInst] << std::endl;
                    currentTime += config->GetStopTime() / nInst;
                }
            }
            else
...
void NativeSolidSolver::setGeneralisedForce(unsigned int instance)
{
    ...
    {
        std::cerr << "Wrong structural type for applying global fluid loads !" << std::endl;
        throw(-1);
    }
}
...
void NativeSolidSolver::setTotalAdjointDisplacement(unsigned int instance)
{
    ...
    std::cout << "AdjDispX: " << AdjDispX << std::endl;
    std::cout << "AdjDispY: " << AdjDispY << std::endl;
    std::cout << "AdjDispZ: " << AdjDispZ << std::endl;
...
void NativeSolidSolver::computeInterfaceAdjointLoads()
{
    ...
    Center[1] = structure->GetCenterOfRotation_y();
    Center[2] = structure->GetCenterOfRotation_z();
    std::cout << "AdjLoads[0]: " << AdjLoads[0] << " AdjLoads[1]: " << AdjLoads[1] << std::endl;
    for (iMarker = 0; iMarker < geometry->GetnMarkers(); iMarker++)
    {
        if (geometry->markersMoving[iMarker] == true)
        {
...
void NativeSolidSolver::computeInterfaceAdjointLoads(unsigned int instance)
{
    ...
    Center[1] = structure->GetCenterOfRotation_y();
    Center[2] = structure->GetCenterOfRotation_z();
    std::cout << "Offset: " << offset << " AdjLoads[0]: " << AdjLoads[instance];
    if (structure->GetnDof() == 2)
        std::cout << " AdjLoads[1]: " << AdjLoads[offset];
    std::cout << std::endl;
    for (iMarker = 0; iMarker < geometry->GetnMarkers(); iMarker++)
    {
        if (geometry->markersMoving[iMarker] == true)
